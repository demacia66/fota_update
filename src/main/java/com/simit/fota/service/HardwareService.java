...
public class HardwareService {
    ...
    public Map<String, String> hardwareLogin(Device device) {
        ...
        String projectName = device.getProject();
    ...
    public Map<String, String> doCheck(CheckVo checkVo) throws Exception {
        ...
        if (!device.getProject().equals(projectName)) {
            res.put("message", "company or projectName error");
        }
        ...
        res.put("update", "true");
        res.put("current_version", curVersion);
        res.put("content", versionDetail.getDescription());
        res.put("date", versionDetail.getCreateTs() + "");
        List<VersionFiles> versionFilesList = versionService.findVersionFile(versionDetail.getFotaProjectID(), versionDetail.getID());
        ...
        if (versionFiles != null) {
            res.put("url", "http://" + hostAddress + ":" + FotaUtil.getLocalPort() + versionFiles.getFileURL());
